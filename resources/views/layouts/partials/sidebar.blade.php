        ...
        <span class="flex h-9 w-9 items-center justify-center rounded-lg bg-gradient-to-br from-sky-500 to-indigo-600 text-white shadow-lg shadow-indigo-900/40">
            ...
            <svg class="h-5 w-5"
                ...
                <path stroke-linecap="round"
                      ...
                      d="M3 15a2 2 0 002 2h.97m0 0a2 2 0 104.06 0m-4.06 0h4.06m0 0h2.25m-2.25 0l1.76-5.28a1 1 0 01.95-.72h1.5a1 1 0 01.95.72L17.06 17m-2.06 0a2 2 0 104.06 0m-4.06 0h4.06M4 11l1-3h4l-1 3M21 9l-1.5 4.5a1 1 0 01-.95.72H18" />
    ...
    @foreach ($navItems as $item)
        ...
        <a href="{{ route($item['route']) }}"
           ...
           class="group flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition
            ...
            <svg class="h-5 w-5 flex-shrink-0 {{ $active ? 'text-sky-400' : 'text-slate-400 group-hover:text-sky-400' }}"
                 fill="none"
                 stroke="currentColor"
                 stroke-width="1.5"
                 viewBox="0 0 24 24">
...
<div class="shrink-0 border-t border-slate-800 p-3">
    ...
    <form method="POST" action="{{ route('logout') }}">
        ...
        <button type="submit"
                class="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-slate-300 transition hover:bg-rose-600/10 hover:text-rose-400">

            <svg class="h-5 w-5 flex-shrink-0 text-slate-400"
                 fill="none"
                 stroke="currentColor"
                 stroke-width="1.5"
                 viewBox="0 0 24 24">
